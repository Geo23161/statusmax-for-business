<template>
  <ion-page v-if="company">
        ...
        <div>
          ...
          <div style="padding: 0.9rem 1rem; text-align: center;">
            <div style="font-size: 1rem; font-weight: bold;">
              Vous n'avez lancé aucune campagne
            </div>
            <div style="font-size: 0.9rem; padding-top: 1rem; ">
              Lancez votre campagne maintenant pour mieux faire connaître votre marque
            </div>
          ...
          <div v-for="post in posts" :key="post.id" @click="router.push('/post/' + post.id)" class="d_sp dem white ">
            ...
            <div class="dem_body">
              <div :style="{
                ...
                'background-image': `url('${post.get_image}')`,
                'margin-right': '1rem',
                'position': 'relative'
              }">
                <button class="play_b" v-if="post.get_media_typ == 'video'">
                  <ion-icon slot="start" :icon="play"></ion-icon>
                </button>
              </div>
              <div class="dem_text">{{ post.name }}</div>
            </div>
            <div class="dem_foot">
              <div>
                <div class="unit">{{ post.get_seen }}</div>
                <div class="sub_unit">vues</div>
              </div>
              <div>
                <div class="unit">{{ post.get_predicted }}</div>
                <div class="sub_unit">vues <br>apres 24h</div>
              </div>
              <div>
                <div class="unit">{{ post.clicks }}</div>
                <div class="sub_unit">clic</div>
              </div>
            </div>
          </div>
    ...
    <pay-comp :is-open="yOpen" @close="yOpen = false" @done="get_update(), yOpen = false" />
...
import { showLoading, my_company, access_tok, show_alert, show_warn } from '@/global/utils';
...
const getActionsButs: (id: number) => ActionSheetButton[] = (id: number) => {
  return [
    ...
    },
    {
      ...
      icon: trash,
      handler() {
        const post = posts.value.filter(e => e.id == id)[0]
        if(post.get_status[0] != "En attente") return setup_alert("Impossible de supprimer", "Cette campagne publicitaire a déjà commencé par être postée, donc nous ne pouvons pas la supprimer")
        deletId.value = id
        show_warn("Confirmation requise", "Etes-vous sûre de vouloir supprimer cette campagne?", "Supprimer", delete_post)
      },
    },
  ];
}
