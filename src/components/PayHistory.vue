<template>
    <ion-modal :is-open="isOpen">
        ...
        <ion-content>
            <div v-if="!compays.length" style="padding-top: 40vh; display: flex; justify-content: space-around;">
                <div v-if="isLoading" >
            ...
            <div v-else>
                <div  class="card_cont">
                    <div v-for="pay in compays" :key="pay.id" class="d_sp">
                        <div class="card">
                            <div class="card_info">
                                <div class="card_name">
                                    {{ format_date(pay.created_at) }}
                                </div>
                                <div>
                                    <div class="stat_">
...
const get_cpays = async () => {
    isLoading.value = true
    const resp = await axios.get('api/get_cpays/', {
        headers : {
            Authorization : `Bearer ${await access_tok(router, undefined)}`
        }
    })
    if(resp.data['done']) compays.value = resp.data['result'];
